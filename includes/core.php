...
function get_images($height, $width, $count, $order) {
	...
	foreach($myimages as $image) {
		...
		echo '<div style="width:' . $width . 'px;"' . $class . '><img alt="rotating image" src="' . RIW_INC_URL . '/genImage.php?height=' . $height . '&amp;width=' . $width . '&amp;ctype=image/jpeg&amp;src=' . wp_get_attachment_url($image->ID) . '" /></div>';
		$i+=1;
	}
}
...
wp_enqueue_style( 'riw-override-css', RIW_INC_URL . '/riw.css', '', '1.0', 'all' );
wp_enqueue_script( 'riw-javascript', RIW_INC_URL . '/riw.js', 'jquery', '1.0', true);
?>